...
def extract_list_references(text: str) -> Dict[int, str]:
    ...
    for line in lines:
        ...
        if in_references and line.startswith('## ') and not re.match(r'^##\s+References?\s*$', line, re.IGNORECASE):
            # Save last entry
...
def corrupt_list_references(content: str, refs: Dict[int, str]) -> str:
    ...
    for line in lines:
        ...
        if in_references_section and line.startswith('## ') and not re.match(r'^##\s+References?\s*$', line, re.IGNORECASE):
            in_references_section = False
    ...
    if in_references_section:
        ...
        shuffled_in_random_order = sorted(shuffle_map.values(), key=lambda x: random.random())
...
def corrupt_citations(content: str, refs: Dict[int, str]) -> str:
    ...
    for line in lines:
        ...
        if in_references_section:
            ...
            if line.startswith('## ') and not re.match(r'^##\s+References?\s*$', line, re.IGNORECASE):
                in_references_section = False
...
def main():
    ...
    print(f"  2. Run test: uv run pytest tests/test_fixtures.py -k {args.fixture_dir.name}")
